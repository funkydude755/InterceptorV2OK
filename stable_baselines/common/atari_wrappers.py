...
class MaxAndSkipEnv(gym.Wrapper):
    ...
    def step(self, action):
        ...
        for i in range(self._skip):
            obs, reward, done, info = self.env.step(action)
            ## CAN ALSO BE HERE TO RAP EVERY THING
            # print ("IN THE ENV WRAPPER:")
            # print("EXPLORING: reward ")
            # print("reward {}".format(reward))
            # print("EXPLORING: action ")
            # print("action {}".format(action))
            #
            # print("EXPLORING: obs (np.ndarray) observation")
            # print("shape: {}, first value: {}, sorted head: {}".format(obs.shape, obs[1],
            #                                                            "****"))
            if i == self._skip - 2:
